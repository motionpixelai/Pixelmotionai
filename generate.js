...
const { image, prompt, style, duration } = body;
    ...
    if (!imageUrl) return res.status(500).json({ error: 'Image upload failed. Try again.' });
    ...
    const klingRes = await fetch('https://api.piapi.ai/api/kling/v1/video', {
      ...
      body: JSON.stringify({
        ...
        input: {
          image_url: imageUrl,
          prompt: `${prompt || 'cinematic motion'}, ${style || 'cinematic'} style`,
    ...
    if (!taskId) return res.status(500).json({ error: 'Could not start video generation. Try again.' });
