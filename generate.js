...
module.exports = async (req, res) => {
  ...
  try {
    const { image, prompt, style, duration, quality, ratio } = req.body;
    ...
    const fullPrompt = `${prompt || 'cinematic motion'}, ${style || 'cinematic'} style, ${duration || '5'}s video`;
    ...
    if (PIAPI_KEY) {
      try {
        ...
        console.log('Kling failed, trying Hugging Face...');
      }
    }
    ...
    if (!hfResponse.ok) {
      if (hfResponse.status === 503) {
        return res.status(503).json({ error: 'AI servers are busy. Please try again in a few minutes.' });
      }
    ...
    return res.status(200).json({ videoUrl: videoDataUrl, source: 'huggingface' });
